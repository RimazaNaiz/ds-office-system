...
const faqKnowledgeBase = [
  {
    ...
    answer:
      'The DS Office Management System is a secure digital platform designed to modernize Divisional Secretariat operations.',
    keywords: ['what', 'ds office', 'system'],
  },
  {
    question: 'What features do you offer?',
    answer:
      'We offer Digital Token System, Citizen Portal, Appointments, Workflow Automation, and Analytics Dashboard.',
    keywords: ['features', 'offer', 'services'],
  },
  {
    question: 'How can I contact support?',
    answer:
      'You can contact us via WhatsApp, phone, or email. Our team is happy to assist you.',
    keywords: ['contact', 'support', 'help'],
  },
];

const quickQuestions = faqKnowledgeBase.map(q => q.question);
...
export default function Chatbot() {
  ...
  const findBestAnswer = (text: string) => {
    const q = text.toLowerCase();
    const match = faqKnowledgeBase.find(f =>
      f.keywords.some(k => q.includes(k))
    );
    return match?.answer || 'Please contact our support team for further assistance.';
  };
  ...
  const handleWhatsApp = () => {
    const phone = '94785706441';
    const message = encodeURIComponent(
      'Hello, I am interested in the DS Office Management System. Please share more details.'
    );
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="fixed bottom-24 right-6 w-96 h-[560px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden z-50"
              ...
              <div className="mt-4 space-y-2">
                {quickQuestions.map(q => (
                  <button
                    ...
                    className="w-full text-left text-sm px-3 py-2 rounded-lg bg-gray-200 hover:bg-gray-300"
                  >
                    {q}
                  </button>
                ))}
